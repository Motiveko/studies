...
<!-- 
  TODO : monorepo에서 eslint config 공유하는법 찾아보자
    - eslint-config-my-app 패키지에다가 index.js로 baseConfig를 넣고, 이거를 각 패키지의 eslint에서 extends 해서 쓸 수 없을까? 일단 임포트가 안되긴한다. 
    - 이렇게 하려는 이유는 하위 패키지에서 뭔가 overrides 같은걸로 .ts에 대한 룰을 재정의 하려고 할 때(ex, naming convention) root에서 .ts에 overrides로 뭔가 해놓으면 통째로 덮어써져버린다.(우선순위가 root가 높다)
    - 하위 eslintrc에서 extends에서 root의 속성을 넣고 그 뒤 override하면 아마 원하는대로 쓸 수 있을 거 같음.
  참고중인 자료
