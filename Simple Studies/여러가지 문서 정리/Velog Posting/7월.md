...
- offsetParent가 참조하는 객체는 생각보다 복잡하다. 보통 position 속성이 지정된 부모 -> table요소, body -> ... 순으로 찾게되는데, div를 부모로써 자식을 감싸고 `transform: rotate`속성을 div에 걸면, div를 offsetParent로 인식하기도 한다. offsetParent를 판단하는 기준은 결국 **자식 요소의 레이아웃 좌표를 결정할 때 참조해서 계산할 기준이 될 부모 요소**인 것이다. 
...
### getBoundingClientRect()의 width, heigth
- https://ohgyun.com/571

<br>

<!-- 
  TODO 요소의 사이즈 구하기(textscroll 에니메이션 구현할 때 getBoundingClientRect().width와 scrollWith 값이 달랐다. 요소 컨텐츠의 실제 너비는 scrollWith였는데, 차이를 잘 모르겠다.
  - https://ohgyun.com/571
  - https://ko.javascript.info/size-and-scroll#ref-680
-->
...
<!-- 
  TODO 2 requestAnimationFrame 조사하기..
 -->