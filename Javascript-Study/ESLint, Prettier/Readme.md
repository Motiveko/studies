...
### 2.1.4 Extending Configuration Files
...
### 2.1.5 Using a configuration from a plugin
...
### 2.1.6  eslint:all, eslint:recommended
...
### 2.2.3 Parser Options
`Parser Options`에서 ESLint에 사용하기 원하는 자바스크립트의 문법 버전을 특정해줄 수 있다. 설정이 없으면 ESLint는 기본 `ECMAScript 5` 문법을 사용한다. 주의할 점은`Enviroment`은 특정 환경의 전역 변수를 인지시켜주는 것이고, `Parser Options`는 문법에 관련된 것이라는 점이다. 예를들어 ES6 문법 사용을 위해선 `{ "parserOptions": { "ecmaVersion": 6 } }`, Set과 같은 ES6 전역 변수 사용을 위해서는 `{ "parserOptions": { "ecmaVersion": 6 } }`의 설정이 필요하다.
...
### 2.5.1 `ignorePatterns` in Config Files
