...
### 13.3 스테이트풀셋
- 쿠버네티스에서 MSA로 동작하는 애플리케이션 pod는 보통 상태가없다. 쿠버네티스는 stateless pod는 특별히 여기지 않아 죽으면 그냥 새로 생성하고 그런다.
- db처럼 상태가 있는(Stateful) 애플리케이션을 쿠버네티스에서 실행시키는건 어려운일이다. 이에 대한 해결책으로 제시되는게 스테이트풀셋이다.
<!-- TODO : 정리중... 크게 할건없다. -->
...
## [데몬셋](https://kimjingo.tistory.com/134)
- 데몬셋은 클러스터 전체 `노드`에 파드를 실행할 때 쓰는 **컨트롤러**다.
- 클러스터에 새로운 노드가 추가되면 데몬셋이 자동으로 해당 노드에 특정 파드를 실행한다. 반대로 노드가 빠질땐 종료되는 파드를 다른 노드에서 재실행하지 않는다.
- 이런 특성은 노드의 ***로그 수집기***나 ***모니터링용 파드***같은걸 스케줄링 할 때 좋다.
