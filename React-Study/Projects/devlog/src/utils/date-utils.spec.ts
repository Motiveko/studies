...
describe('date util 테스트', () => {
  test('테스트', () => {
    ...
    // 3. ~ 시간 전
    [...Array(23).keys()].map(i => i + 1).forEach(i => expect(parseDate(nowMillis - i * oneHourMillis)).toBe(`${i}시간 전`));

    // 4. ~ 일 전
    [...Array(6).keys()].map(i => i + 1).forEach(i => expect(parseDate(nowMillis - i * oneDayMillis)).toBe(`${i}일 전`));
    ...
    expect(parseDate(new Date(anotherYearDate).getTime())).toBe(anotherYearDate);
  });
});
