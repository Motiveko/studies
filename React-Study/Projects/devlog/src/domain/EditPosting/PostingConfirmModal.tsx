import React, { ChangeEvent, ChangeEventHandler, KeyboardEvent, KeyboardEventHandler, SetStateAction, useCallback, useRef } from 'react';
...
function PostingConfirmModal({ show, setShow }: props) {
  ...
  /** 썸네일 제거 */
  const removeThumbnail = useCallback(() => mergePosting({ thumbnail: '' }), [mergePosting]);

  const tagInputRef = useRef<HTMLTextAreaElement>(null);

  /** 태그 제거 */
  const removeTag = useCallback(index => mergePosting({ tags: posting.tags.filter((e, i) => i !== index) }), [mergePosting, posting.tags]);
  /**
   ...
   * 설명 변경
   */
  const onDescChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    ...
    <Modal show={show}>
      ...
      <Modal.Body>
        <div className="d-flex">
          ...
          {posting.thumbnail && <IconButton icon={faTimes} onClick={removeThumbnail} />}
        </div>
        <ImageEditor isLoading={localLoading} thumbnail={posting.thumbnail || ''} handleChange={handleFileChange} variant="thumbnail" />

        <h6 className="mt-2">포스트 설명</h6>
        <Form.Control as="textarea" onKeyPress={prohibitEnter} onChange={onDescChange} ref={descRef} style={{ resize: 'none' }} value={posting.description} />
      ...
      <Modal.Footer>
        <div className="d-flex justify-content-end mt-2" style={{ height: '4vh' }}>
          ...
          <Button className="ms-2" size="sm" variant="success" onClick={uploadPosting}>
            출간하기
          </Button>
        </div>
      </Modal.Footer>
